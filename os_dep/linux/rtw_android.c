...
int rtw_android_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	if (copy_from_user(command, (void *)(long int)priv_cmd.buf, priv_cmd.total_len)) {
		ret = -EFAULT;
		goto exit;
	}
	...
	case ANDROID_WIFI_CMD_WFD_SET_TCPPORT:
	{
		...
		pwfd_info = &padapter->wfd_info;
		if( padapter->wdinfo.driver_interface == DRIVER_CFG80211 )
			pwfd_info->rtsp_ctrlport = ( u16 ) get_int_from_command( (char *)(long int)priv_cmd.buf );
		break;
	}
	...
	case ANDROID_WIFI_CMD_WFD_SET_DEVTYPE:
	{
		...
		if( padapter->wdinfo.driver_interface == DRIVER_CFG80211 )
		{
			pwfd_info->wfd_device_type = ( u8 ) get_int_from_command( (char *)(long int)priv_cmd.buf );
...
response:
	if (bytes_written >= 0) {
		...
		if (copy_to_user((void *)(long int)priv_cmd.buf, command, bytes_written)) {
